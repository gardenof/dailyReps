...
createGymRecord :: String -> Int -> Gym
createGymRecord name level =
  Gym { gymName = name, gymLevel = level }

gymConstructor :: String -> Int -> Gym
gymConstructor = Gym

gymConstructorReg :: String -> Int -> Gym
gymConstructorReg name year = Gym name year

gymUpdateName :: String -> Gym -> Gym
gymUpdateName newNameStr gym =
  gym {gymName = newNameStr}
...
vegToColor vegetable =
  case vegetable of
    Celery -> Green
    Carrot color -> color

vegToString :: Vegetable -> String
vegToString vegetable =
  colorToString $ vegToColor vegetable
...
nameToString (Name nameStr) = nameStr
...
personJim :: Person
personJim = Person (Name "Jeje") (Hometown "Westwing") (Age 33)
