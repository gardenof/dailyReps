module Year2020.Month05.Day13 () where
...
instance Eq Name where
  left == right =
    let
      nameleft = map Char.toLower (nameToString left)
      nameright = map Char.toLower (nameToString right)
    in
      nameleft == nameright
