...
gymInstanceRecord :: String -> Int -> Gym
gymInstanceRecord name level =
...
gymConstructorReg name level = Gym name level

updateLevel :: Int -> Gym -> Gym
updateLevel newLevel gym = gym {gymLevel = newLevel}
...
instance Eq Car where
  left == right =
    carName left == carName right
    && carYear left == carYear right
