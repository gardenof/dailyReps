module Year2020.Month06.Day11 () where
...
personOne :: Person
personOne = Person(Name"Jim")(HomeTown"Moon")(Age 22)
...
comparePlayers1 p1 p2 =
  case Ord.comparing playerName p1 p2 of
    GT -> GT
    LT -> LT
    EQ -> Ord.comparing playerRank p1 p2
...
dollarsX2 :: Int
dollarsX2 =
  length . words $ concat $ ["one nad ","Two"]

dollarsX1 :: Int
dollarsX1 = length . words . concat $ ["one and ","Two"]

dollarsX0 :: Int
dollarsX0 = (length . words . concat) ["one nad ","Two"]
...
wordsInHaikuTwo :: [Int]
wordsInHaikuTwo = map (length . words) $ lines haiku
