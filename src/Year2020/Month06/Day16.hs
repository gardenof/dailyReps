module Year2020.Month06.Day16 () where
...
personOne :: Person
personOne = Person(Name"Jim")(HomeTown"Moon")(Age 33)
...
ranks :: [Rank]
ranks = [Gold, Silver, Bronze, Bronze]

ascendingRanks :: [Rank]
ascendingRanks = List.sort ranks

descinding :: [Rank]
descinding = List.sortOn Ord.Down ranks
...
alphabeticalPlayers = List.sortOn playerName players

rankedPlayers :: [Player]
rankedPlayers = List.sortBy (Ord.comparing playerRank) players
...
nameLengths :: [Int]
nameLengths = map length names

lengthsSum :: Int
lengthsSum = sum nameLengths

roster :: String
roster = List.intercalate ", " names

isNameLonger :: String -> Bool
isNameLonger string = length string > 7
